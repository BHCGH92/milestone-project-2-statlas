 ...
 * @returns {string | undefined} - A random country name from the list, or undefined if the array is empty.
 */

export function getRandomCountry(countryNames) {
    if (!countryNames || countryNames.length === 0) {
    console.warn("Something went wrong - getRandomCountry was called with an empty array.");
    return undefined;
  }
 ...
 * @returns {Promise<object>} A promise that resolves to the transformed data of a random country.
 */
...
export async function fetchRandomCountry() {
    try {
        ...
        // If any step fails, log it and pass the error up to the event listener.
